...
#
# Created by etl() on 2026-09-09 21:11:20
...
if [[ $do_update = 1 ]]; then
  ...
  attempts=0
  until (
    wget --retry-connrefused --waitretry=1 --read-timeout=20 --timeout=15 -t 10 -c -O download/ma_2022_svi_tract.zip 'https://svi.cdc.gov/Documents/Data/2022/db/states/Massachusetts.zip' 2>&1
  ); do
    ((attempts++))
    if (( attempts > 3 )); then echo $?; break; fi
  done
  unzip -o download/ma_2022_svi_tract.zip -d download && rm download/ma_2022_svi_tract.zip 2>&1
