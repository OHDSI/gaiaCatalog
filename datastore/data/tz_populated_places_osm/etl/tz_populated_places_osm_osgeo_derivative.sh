...
ogr2ogr -f GeoJSON -overwrite derived/tz_populated_places_osm.json PG:"dbname=$POSTGRES_DB port=$POSTGRES_PORT user=$POSTGRES_USER password=$POSTGRES_PASSWORD host='gaia-db'" -select "osm_id,name,place,other_tags,geom" tz_populated_places_osm

# Create downloadable tarfile of geojson in derivative directory on osgeo
rm -f derived/tz_populated_places_osm.geojson.tar.gz
tar -czf derived/tz_populated_places_osm.geojson.tar.gz meta_dcat_tz_populated_places_osm.json -C derived tz_populated_places_osm.json
