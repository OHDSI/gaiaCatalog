...
# us_2023_annual_aqi_by_county_postgis.sh
...
pg_dump -d $POSTGRES_DB -U $POSTGRES_USER -p $POSTGRES_PORT -h gaia-db -t us_2020_census_county_fips | psql -d $POSTGRES_DB -U $POSTGRES_USER -p $POSTGRES_PORT -h gaia-db
...
ALTER TABLE us_2023_annual_aqi_by_county ADD COLUMN \"90th_percentile_aqi\" int4;
UPDATE us_2023_annual_aqi_by_county SET \"90th_percentile_aqi\"=temp.\"90th_percentile_aqi\"::int4
  FROM temp
  WHERE us_2023_annual_aqi_by_county.geoid = temp.geoid;
...
ALTER TABLE us_2023_annual_aqi_by_county ADD COLUMN \"days_pm2.5\" int4;
UPDATE us_2023_annual_aqi_by_county SET \"days_pm2.5\"=temp.\"days_pm2.5\"::int4
  FROM temp
  WHERE us_2023_annual_aqi_by_county.geoid = temp.geoid;
